...
set(gca,'FontSize',20); % text fontsize

text(x0, y0, 'x=0,y=0', 'FontSize',16); % text by a point


s = sprintf('x=%2.0f, y=%2.1f',x0, y0);
text(x0+0.3, y0, s, 'FontSize',16); % sprintf by a point
...
legend('\alpha=1', '\alpha=2', '\alpha=4'); %label
hold off
text(pi/2, 1, '\alpha=1');
text(pi/2, 2, '\alpha=2');
text(pi/2, 4, '\alpha=4');

% please plot before text！

%%%%%%%%%% ex: label by the anime %%%%%%%%%%
close all; clf; clear
